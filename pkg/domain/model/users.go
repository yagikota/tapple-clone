...
type userR struct {
	Hobbies           HobbySlice            `boil:"Hobbies" json:"Hobbies" toml:"Hobbies" yaml:"Hobbies"`
	...
	UserProfileImages UserProfileImageSlice `boil:"UserProfileImages" json:"UserProfileImages" toml:"UserProfileImages" yaml:"UserProfileImages"`
}
...
func (r *userR) GetHobbies() HobbySlice {
	if r == nil {
		return nil
	}
	return r.Hobbies
...
func (r *userR) GetUserProfileImages() UserProfileImageSlice {
	if r == nil {
		return nil
	}
	return r.UserProfileImages
}
...
// Hobbies retrieves all the hobby's Hobbies with an executor.
func (o *User) Hobbies(mods ...qm.QueryMod) hobbyQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`hobbies`.`user_id`=?", o.ID),
	)

	return Hobbies(queryMods...)
}
...
// UserProfileImages retrieves all the user_profile_image's UserProfileImages with an executor.
func (o *User) UserProfileImages(mods ...qm.QueryMod) userProfileImageQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("`user_profile_images`.`user_id`=?", o.ID),
	)

	return UserProfileImages(queryMods...)
}

// LoadHobbies allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadHobbies(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`hobbies`),
		qm.WhereIn(`hobbies.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load hobbies")
	}

	var resultSlice []*Hobby
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice hobbies")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on hobbies")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for hobbies")
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.UserID {
				local.R.Hobbies = append(local.R.Hobbies, foreign)
				if foreign.R == nil {
					foreign.R = &hobbyR{}
				}
...
func (userL) LoadRoomUsers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`room_users`),
		qm.WhereIn(`room_users.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// LoadUserProfileImages allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadUserProfileImages(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`user_profile_images`),
		qm.WhereIn(`user_profile_images.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load user_profile_images")
	}

	var resultSlice []*UserProfileImage
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice user_profile_images")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on user_profile_images")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for user_profile_images")
	}
	...
	if singular {
		object.R.UserProfileImages = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &userProfileImageR{}
			}
			foreign.R.User = object
		}
		return nil
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.UserID {
				local.R.UserProfileImages = append(local.R.UserProfileImages, foreign)
				if foreign.R == nil {
					foreign.R = &userProfileImageR{}
				}
				foreign.R.User = local
...
// AddHobbiesG adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.Hobbies.
// Sets related.R.User appropriately.
// Uses the global database handle.
func (o *User) AddHobbiesG(ctx context.Context, insert bool, related ...*Hobby) error {
	return o.AddHobbies(ctx, boil.GetContextDB(), insert, related...)
}

// AddHobbies adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.Hobbies.
// Sets related.R.User appropriately.
func (o *User) AddHobbies(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Hobby) error {
	var err error
	...
	if o.R == nil {
		...
		o.R.Hobbies = append(o.R.Hobbies, related...)
	}
...
// AddRoomUsersG adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.RoomUsers.
// Sets related.R.User appropriately.
// Uses the global database handle.
func (o *User) AddRoomUsersG(ctx context.Context, insert bool, related ...*RoomUser) error {
	return o.AddRoomUsers(ctx, boil.GetContextDB(), insert, related...)
}
...
// AddUserProfileImagesG adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.UserProfileImages.
// Sets related.R.User appropriately.
// Uses the global database handle.
func (o *User) AddUserProfileImagesG(ctx context.Context, insert bool, related ...*UserProfileImage) error {
	return o.AddUserProfileImages(ctx, boil.GetContextDB(), insert, related...)
}

// AddUserProfileImages adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.UserProfileImages.
// Sets related.R.User appropriately.
func (o *User) AddUserProfileImages(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*UserProfileImage) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.UserID = o.ID
	...
	if o.R == nil {
		o.R = &userR{
			UserProfileImages: related,
		}
	} else {
		o.R.UserProfileImages = append(o.R.UserProfileImages, related...)
	}

	for _, rel := range related {
		if rel.R == nil {
			rel.R = &userProfileImageR{
				User: o,
			}
		} else {
			rel.R.User = o
